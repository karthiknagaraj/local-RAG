...
---
## 10) Next actions (I can do these)
- Build the **cost-model spreadsheet** and add it to `docs/` (I can pre-populate with example scenarios).  
- Create a **Jira-MVP spec** (I can generate `docs/JIRA_MVP.md` with the ingestion config and test queries).  
- Implement an automated **daily cost report** that emails/alerts when spend exceeds budget.

Tell me which one you want first and any numbers to plug into the cost model (expected queries/day, avg prompt/response token counts, preference for managed vs self-hosted).